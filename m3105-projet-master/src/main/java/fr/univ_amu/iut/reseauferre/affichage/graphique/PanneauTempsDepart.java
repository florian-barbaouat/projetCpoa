...
public class PanneauTempsDepart {
    @FXML
    ...
    private void generateWrapper(ObservableList<WrapperAffichageTemps> affichage) {
        ...
        for (Trajet trajet : donnees.getAllTrajet()) {
            for (Sillon sillon : trajet.getSillonsEmprunte()) {
                affichage.add(new WrapperAffichageTemps(sillon.getLigneFerroviaire().getDepart(), tempsRestant(sillon.getHeure(), donnees.getHeureActuel()), trajet.getDepart(), trajet.getDestination()));
            }
        }
    }
